...
function revealPage() {
  ...
  const tl = gsap.timeline({
    onComplete: () => {
      ...
      // Re-enable scroll after preloader closes
      document.body.style.overflow = '';
      if (lenis) lenis.start();
    }
  });

  // Temporarily disable scroll during intro
  ...
  // Pulse Hero Stat dividers
  tl.from('.hero-stat-row .stat-item', {
    opacity: 0,
    y: 20,
    duration: 1.0,
    stagger: 0.1,
